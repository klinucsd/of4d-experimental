"""
CH-GEE Replication for SMALL areas - Direct local download only
...
def split_bbox_into_tiles(bbox, tile_size_deg=0.1):
    """
    Split large bbox into smaller tiles that can be downloaded
    ...
    tile_size_deg : float
        Size of each tile in degrees (0.1° = ~11km at equator)
    
    Returns:
    --------
    list of tuples: [(tile_bbox, tile_name), ...]
    """
    ...
    while lon < max_lon:
        ...
        while lat < max_lat:
            ...
            tiles.append((tile_bbox, f"tile_{tile_i}_{tile_j}"))
...
if __name__ == "__main__":
    ...
    # Resolution - automatically adjust based on area size
    area_deg2 = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
    
    # Calculate max safe resolution based on GEE 50MB limit
    # Rule of thumb: area_deg2 * 111000^2 / scale^2 * 5 bands * 4 bytes < 50MB
    max_safe_scale = int(np.sqrt(area_deg2 * 111000**2 * 5 * 4 / (50 * 1024 * 1024)))
    
    if area_deg2 > 0.25:  # > 0.25 deg² (3,000 km²)
        scale = 30
        print("⚠ Large area detected, using 30m resolution")
    elif area_deg2 > 0.04:  # 0.04-0.25 deg² (500-3,000 km²)
        scale = 20
        print("Medium area, using 20m resolution")
    else:  # < 0.04 deg² (<500 km²)
        scale = 10
        print("Small area, using 10m resolution")
    
    print(f"  (Recommended minimum scale for this area: {max_safe_scale}m)")
    
    # Or manually override (but may fail if too large):
    # scale = 10  # Force 10m - only works for bbox < 0.1° x 0.1°
    ...
    print("="*60)
    print(f"\nArea: {area_km2:.0f} km²")
    print(f"Resolution: {scale}m (automatically selected)")
    print(f"  - Use scale=10 for 10m (force high-res)")
    print(f"  - Use scale=20 for 20m (balanced)")
    print(f"  - Use scale=30 for 30m (faster/smaller)")
